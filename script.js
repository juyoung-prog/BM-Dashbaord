...
function restoreDashboard() {
  ...
  const audStats = document.getElementById('audience-stats-grid');
  if (audStats) audStats.innerHTML = `
    <div class="stat-card"><h4>Black Community</h4><div class="stat-big" style="color:#6366F1">${blackStores.length}</div><div class="stat-sub">stores with &gt;30% Black population</div></div>
  ...
  if (audList) {
    const segs = [
      { name:'Black Hair Care Community', color:'#6366F1', stores: STORES.filter(s=>s.black>=40) },
...
function rowToStore(row, id) {
  ...
  // raceBar
  const raceBar = [];
  if (black > 5)  raceBar.push({ w: black, c: '#4F46E5' });
  if (hisp  > 5)  raceBar.push({ w: hisp,  c: '#E81D25' });
  if (asian > 5)  raceBar.push({ w: asian, c: '#F59E0B' });
  if (white > 5)  raceBar.push({ w: white, c: '#E5E7EB' });
  // Priority logic
...
function computeKPIsFromStores(stores) {
  ...
  return {
    ...
    black:     { val: String(blackMaj.length), meta: 'stores >50% Black',    color: '#6366F1',         barW: `${Math.round(blackMaj.length/n*100)}%`, barC: '#6366F1' },
    ...
    asian:     { val: String(asianHigh.length),meta: 'stores >20% Asian',    color: '#F59E0B',         barW: `${Math.round(asianHigh.length/n*100)}%`, barC: '#F59E0B' },
  };
}
