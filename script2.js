...
const ALL_KPI_DEFS = {
  ...
  asian:     { label:'High Asian Population',  value:'2',      meta:'stores >20% Asian pop.', color:'#D97706',               barW:'13%', barC:'#D97706'        },
};
...
function rowToStore(row, id) {
  ...
  if (hisp  > 5)  raceBar.push({ w: hisp,  c: '#059669' });
  if (asian > 5)  raceBar.push({ w: asian, c: '#D97706' });
...
function computeKPIsFromStores(stores) {
  ...
  return {
    ...
    asian:     { val: String(asianHigh.length),meta: 'stores >20% Asian',    color: '#D97706',         barW: `${Math.round(asianHigh.length/n*100)}%`, barC: '#D97706' },
  };
}
