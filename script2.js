...
const ALL_KPI_DEFS = {
  ...
  black:     { label:'Black-Majority Markets', value:'4',      meta:'stores >50% Black pop.', color:'#2563EB',              barW:'27%', barC:'#2563EB'        },
  ...
  asian:     { label:'High Asian Population',  value:'2',      meta:'stores >20% Asian pop.', color:'#16A34A',               barW:'13%', barC:'#16A34A'        },
};
...
function restoreDashboard() {
  ...
  const audStats = document.getElementById('audience-stats-grid');
  if (audStats) audStats.innerHTML = `
    <div class="stat-card"><h4>Black Community</h4><div class="stat-big" style="color:#2563EB">${blackStores.length}</div><div class="stat-sub">stores with &gt;30% Black population</div></div>
  ...
  if (audList) {
    const segs = [
      { name:'Black Hair Care Community', color:'#2563EB', stores: STORES.filter(s=>s.black>=40) },
...
function rowToStore(row, id) {
  ...
  // raceBar
  const raceBar = [];
  if (black > 5)  raceBar.push({ w: black, c: '#2563EB' });
  if (hisp  > 5)  raceBar.push({ w: hisp,  c: '#0891B2' });
  if (asian > 5)  raceBar.push({ w: asian, c: '#16A34A' });
  ...
  if (twoPlus > 3) raceBar.push({ w: twoPlus, c: '#9333EA' });
...
function computeKPIsFromStores(stores) {
  ...
  return {
    ...
    black:     { val: String(blackMaj.length), meta: 'stores >50% Black',    color: '#2563EB',         barW: `${Math.round(blackMaj.length/n*100)}%`, barC: '#2563EB' },
    ...
    asian:     { val: String(asianHigh.length),meta: 'stores >20% Asian',    color: '#16A34A',         barW: `${Math.round(asianHigh.length/n*100)}%`, barC: '#16A34A' },
  };
}
